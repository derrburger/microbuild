...
| Buyer selection | **My Requests v2** (`/dashboard/requests`, `#buyer-my-requests-applicants`) — summary cards, filters, scannable request cards, expandable details, **AI Request Monitor** (rules-based), applicant review, selected creator card, project timeline. Legacy **`MarketplaceApplicantsPanel`** re-exports the new panel. Selection + messaging unchanged (`selectCreatorForRequest`, central `/messages`). **No payment/proposal expansion in this phase.** |
...
### Buyer My Requests v2 (`/dashboard/requests`)
