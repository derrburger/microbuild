...
| Admin | **`/admin`** buyer request cards include a **marketplace strip** (applicant totals, marketplace lifecycle, buyer-selected badge, selected application id excerpt). Pipeline / manual assignment stays **fallback**, not the default path. **Workflow AI overview** unchanged. `/admin` still does not expose raw **`project_messages`** content. |
